...
wifi_lock = wifi_service.createWifiLock(
...
def get_network_status():
    ...
    connectivity_service = activity.getSystemService(
...
def create_shortcut(data, label):
    ...
    if shortcut_manager.isRequestPinShortcutSupported():
        ...
        shortcut_intent.setClassName(
            "tw.avianjay.taiwanbusflet",
            "tw.avianjay.taiwanbusflet.MainActivity"
        )
        shortcut_intent.setData(Uri.parse(data))
        shortcut_intent.setFlags(
            Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK
        )

        # 4. 建立 ShortcutInfo
        ShortcutInfoBuilder = autoclass(
        ...
        pinned_intent = shortcut_manager.createShortcutResultIntent(
            shortcut_info
        )
